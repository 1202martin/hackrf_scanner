...
class Scanner:
    ...
    scan_flag = True
    #initiate hackrf_sweep to collect information on provided central frequency.
    ...
    def update_db(self,DB_CONFIG,freq_info,init_flag):
        #Connect to DB
    ...
    def freq_scan(self,DB_CONFIG, central_freq, bw, bin, init_flag):
        ...
        while True:
            ...
            self.update_db(self,DB_CONFIG, central_freq, [freq_info], init_flag)
            print("Elapase : ", time.time()-startTime)
            if self.scan_flag == False:
                break

    def freq_scan_start_end(self,DB_CONFIG, min_freq, max_freq, bin, init_flag):
        #This function continuosly scans the selected frequency range && updates this data to the local DB
        #This function triggers both the frequency scan function along with the db update function.
        ...
        while self.scan_flag:
            ...
            self.update_db(self,DB_CONFIG, [freq_info], init_flag)
            print("elapse: ", time.time()-start_time)

    def init_table(self,DB_CONFIG):
        self.scan_flag = False
        self.freq_scan(self,DB_CONFIG,2716, 4606, 1, 1)